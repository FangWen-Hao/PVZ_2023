...
{
	if (point.x < (bee.Left() + bee.Width()) && point.x > bee.Left() 
		&& point.y < (bee.Top() + bee.Height()) && point.y > bee.Top()
		&& !bee.IsAnimation())
	{
		bee.SetAnimation(2, false);
	}
	
	if (point.x < (ball.Left() + ball.Width()) && point.x > ball.Left()
		&& point.y < (ball.Top() + ball.Height()) && point.y > ball.Top()
		&& !ball.IsAnimation())
	{
		ball.SetAnimation(3000, true);
